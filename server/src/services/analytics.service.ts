...
export const chartAnalyticsService = async (
  ...
  const result = await TransactionModel.aggregate([
    { $match: filter },
    //Group the transaction by date (YYYY-MM-DD) using UTC timezone for consistent grouping
    {
      $group: {
        _id: {
          $dateToString: {
...
export const expensePieChartBreakdownService = async (
  ...
  const hasSairCategory = data.breakdown.some(
    (item: any) => item.name === "سایر"
  );

  const othersEntry = data.breakdown.find(
    (item: any) => item.name === "others"
  );

  if (hasSairCategory && othersEntry) {
    const transformedBreakdown = data.breakdown
      .filter((item: any) => item.name !== "others")
      .map((item: any) => {
        if (item.name === "سایر") {
          return {
            name: item.name,
            value: item.value + othersEntry.value,
            percentage:
              data.totalSpent === 0
                ? 0
                : Math.round(
                    ((item.value + othersEntry.value) / data.totalSpent) * 100
                  ),
          };
        }
        return {
  ...
  const transformedData = {
    ...
    breakdown: data.breakdown.map((item: any) => ({
      ...item,
      name: item.name === "others" ? "سایر" : item.name,
    })),
  };
